...
class StiViewer extends StiHtmlComponent {
    ...
    public function getHtml($element = null) {
        ...
        $result .= "$viewerProperty.showProcessIndicator();\n";
        ...
        $result .= "$viewerProperty.onShowReport = function (event) {console.log(\"Report Begin Process Data\");}\n";
//        
        ...
        $element = is_null($element) ? '' : $element;
        $result .= "$viewerProperty.renderHtml(" . (strlen($element) > 0 ? "'$element'" : '') . ");\n";

        $result .= "var userButton = viewer.jsObject.SmallButton(\"userButton\", \"Tutup\", \"emptyImage\");\n";
        $result .= "userButton.image.src = 'assets/images/exit.png';\n";
        $result .= "userButton.action = function () {window.close();}\n";
        $result .= "var toolbarTable = viewer.jsObject.controls.toolbar.firstChild.firstChild;\nvar buttonsTable = toolbarTable.rows[0].lastChild.lastChild;\nvar userButtonCell = buttonsTable.rows[0].insertCell(0);\n";
        $result .= "userButtonCell.className = \"stiJsViewerClearAllStyles\";\n";
        $result .= "userButtonCell.appendChild(userButton);\n";
        return $result;
    }
